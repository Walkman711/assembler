...
use crate::{error::AssemblerError, instructions::Instruction, mnemonics::BranchMnemonic};
use std::{
...
pub fn assemble_file(filename: &str) -> Result<(), AssemblerError> {
    let (preprocessed_program, _strings) = preprocess_asm_file(&filename)?;
